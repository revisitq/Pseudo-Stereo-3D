...
class pseudo_feature_extraction_neck(nn.Module):
    def __init__(self, cfg):
        super(pseudo_feature_extraction_neck, self).__init__()
        ...
        if self.with_upconv:
            # assert self.start_level == 2
            self.upconv_module = upconv_module([concat_dim], [64, 32])
            stereo_dim = 32
        else:
            stereo_dim = concat_dim
            # assert self.start_level >= 1
    ...
    def forward(self, feats):
        ...
        stereo_feature = concat_feature
        if self.with_upconv:
            stereo_feature = self.upconv_module([stereo_feature])
        stereo_feature = self.lastconv(stereo_feature)
        if self.cat_img_feature:
            sem_feature = self.rpnconv(concat_feature)
        else:
